...
def get_car_id(make, model):
    car = (
        Session.query(Car.id).filter(Car.make_fa == make, Car.model_fa == model).first()
    )
    if not car:
        return None
    return car.id
...
def parse_ads(session):
    try:
        ...
        subquery = (
            session.query(RawAd.ad_code, func.max(RawAd.version).label("max_version"))
            .group_by(RawAd.ad_code)
            .subquery()
        )

        raw_ads = (
            session.query(RawAd.id, RawAd.raw_data, RawAd.ad_code)
            .join(
                subquery,
                (RawAd.ad_code == subquery.c.ad_code)
                & (RawAd.version == subquery.c.max_version),
            )
        ...
        for raw_ad in raw_ads:
            ...
            if raw_data is not None:
                    ...
                    dealer_stmt = (
                        ...
                        .on_conflict_do_update(
                            ...
                            set_={
                                ...
                                "address": raw_data.get("dealer", {}).get("address"),
                                "ad_count": raw_data.get("dealer", {}).get("ad_count"),
                                "package_type": raw_data.get("dealer", {}).get(
                                    "package_type"
                                ),
                            },
                        )
                ...
                session.query(RawAd).filter(RawAd.id == raw_id).update(
                    {"process_at": datetime.now(pytz.utc)}
                )
        session.commit()
